...
from transformers import (WEIGHTS_NAME, get_linear_schedule_with_warmup, AdamW,
                          ...
                          XLMRobertaTokenizer, LongformerConfig, LongformerForSequenceClassification, LongformerTokenizer)

from utils import (compute_metrics, convert_examples_to_features,
                        output_modes, processors)

from type_classification.arguments import add_arguments

logger = logging.getLogger(__name__)

MODEL_CLASSES = {'roberta': (RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer),
                 'xlm': (XLMRobertaConfig, XLMRobertaForSequenceClassification, XLMRobertaTokenizer),
...
def evaluate(args, model, tokenizer, checkpoint=None, prefix="", mode='dev'):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        pred_by_ex = np.array([np.bincount(preds_label[example_ids==i]).argmax() for i in all_example_ids])
        labels_by_ex = np.array([np.bincount(out_label_ids[example_ids==i]).argmax() for i in all_example_ids])
        assert len(pred_by_ex) == len(labels_by_ex)

        logger.info("Total Example length: %d", len(pred_by_ex))
        logger.info("Pred by ex:\n%s", pred_by_ex)
        logger.info("Label by ex:\n%s", labels_by_ex)

        result = compute_metrics(eval_task, pred_by_ex, labels_by_ex, args.model_type)
...
def load_and_cache_examples(args, task, tokenizer, ttype='train'):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}_{}_full'.format(
        ttype,
        file_name,
...
def main(args):
    ...
    if args.tokenizer_name and args.tokenizer_name == 'xlm-roberta-base':
